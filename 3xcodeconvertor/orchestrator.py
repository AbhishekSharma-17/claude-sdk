"""Pipeline orchestrator: Phase 1 → 2 → 3 → 4.
...
from phases import conversion, discovery, planning, validation
...
async def run_pipeline(config: ConverterConfig) -> ConversionReport:
    ...
    Phase 4: Validation — syntax check + Claude review
    """
    ...
    # Discover files (could parallelize, but usually few files)
    ...
    if total_objects == 0:
        ...
        return ConversionReport(
            file_complexity="No objects found",
            cost_breakdown=cost,
        )
    ...
    # Budget check before conversion
    estimated_conversion_cost = total_objects * 1.75  # rough estimate
    ...
    # Filter out already-checkpointed objects
    ...
    validation_results = await validation.validate_all(output_files, validation_options, config)
    ...
    logger.info("=" * 60)
    logger.info("  Objects: %d converted, %d failed, %d skipped",
                 report.converted, report.failed, report.skipped)
    logger.info("  Cost: $%.4f (discovery: $%.4f, planning: $%.4f, conversion: $%.4f, validation: $%.4f)",
                 cost.total, cost.discovery, cost.planning, cost.conversion, cost.validation)
...
def _save_dry_run_output(
    ...
    """Save discovery and planning results for dry run."""
    output = {
        "mode": "dry_run",
